...
-(void)setData:(NSDictionary *)data index:(NSInteger) index {
    ...
    NSArray *title = @[@"ProjectName",@"AlarmName",@"TimeStamp",@"Status"];
    for (int i = 0; i < title.count; i++) {
        ...
        lab.numberOfLines = 0;
        if (i == 2) {
            lab.text = [NSString stringWithFormat:@"%@",[TTUtils dateUTCToNow:data[title[i]] oldSf:@"yyyy-MM-dd'T'HH:mm:ss.SSS'Z'" nowSf:@"yyyy-MM-dd HH:mm:ss"]];
        }
